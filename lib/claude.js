...
export async function roastResume(resumeText) {
  ...
  const rawText = await callProvider(provider, resumeText); // network/auth errors bubble up
  ...
  try {
    ...
    console.error(`Roast validation failed (provider=${provider}), using fallback:`, err?.message);
    return fallback();
  }
}
